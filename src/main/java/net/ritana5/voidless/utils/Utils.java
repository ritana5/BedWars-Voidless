package net.ritana5.voidless.utils;
...
public class Utils {
    ...
    public static void info(String text) {
        Bukkit.getConsoleSender().sendMessage("[BedWars-Voidless] " + color(text));
    }

    public static void sendActionBar1058(Player player, String message) {
        JavaPlugin.getPlugin(Voidless.class);
        Voidless.bw1058Api.getVersionSupport().playAction(player, color(message));
    }

    public static void sendActionBar2023(Player player, String message) {
        JavaPlugin.getPlugin(Voidless.class);
        Voidless.bw2023Api.getVersionSupport().playAction(player, color(message));
    }
